...
ls -1 *.bedGraph|while read i; do bedGraphToBigWig $i /path/to/reference/S_lycopersicum_chromosomes.4.00.chrom.sizes $i.bw; done
...
ls -1 *.bw | while read i; do
    ...
    computeMatrix scale-regions \
        ...
        --skipZeros \
        -R /path/to/reference/bed/gene.bed \
        -S $i \
        --missingDataAsZero \
        -o $i.gene_body.gz \
