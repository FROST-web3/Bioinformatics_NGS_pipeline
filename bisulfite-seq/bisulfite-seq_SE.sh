...
process_methylation_data() {
    ...
    # fastp
    mkdir cleanData/
    ls -1 *.fastq |while read i; do 
        echo "fastp -i $i  -o cleanData/${i%.fa*}_clean.fq  --detect_adapter_for_pe -f 6 -t 6  -w 28 -q 18 -u 30 -n 20 --length_required 30 ">>fastp.sh
    done
    sh fastp.sh 
    rm -rf *.fastq
    ...
    ls -1 *.fq |while read i j; do 
        echo "bismark --genome /path/to/reference/ref --parallel 5 -o bismark_alignment/  $i  --bowtie2 --gzip">>bismark.sh
    done
    sh bismark.sh
